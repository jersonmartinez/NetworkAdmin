...
function backupDHCP() {
    ...
    if [[ $paquete != '' ]]; then
        ...
        echo -e "\n\e[0;31mEl servicio no está instalado en el servidor indicado\n"
...
function backupDNS() {
    ...
    if [[ $paquete != '' ]]; then
        ...
        echo -e "\n\e[0;31mEl servicio DNS no está instalado el servidor\n"
	...
	if [ $# == 1 ]; then
		...
		case ${ListArguments[$CONTADOR]} in
		    ...
		    $data)
			    ...
			    #     mkdir /var/data/$date_dir
			    # fi
			    # scp -C -r $username@$IPAddress:$DirData /var/data/$date_dir/
			    
			    # cd /var/data/$date_dir/
			    # tar -czvf $HostName"_"$date_file"_"$hour.tar.gz $DirData
			    # rm -rf $DirData
		    ;;
		    *)
